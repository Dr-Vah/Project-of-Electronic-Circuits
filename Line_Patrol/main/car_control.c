...
esp_err_t car_control_strafe_left(float speed_mps)
{
    ...
    ESP_RETURN_ON_ERROR(
        car_control_set_wheel_speed(CAR_WHEEL_B,
                                    STRAFE_SIDE_SPEED_RATIO * speed_mps),
        "car_control", "set left wheel speed failed");
    return car_control_set_wheel_speed(
        CAR_WHEEL_D, STRAFE_SIDE_SPEED_RATIO * speed_mps);
}
...
esp_err_t car_control_stop(void)
{
    ...
    for (int i = 0; i < CAR_WHEEL_COUNT; ++i) {
        ESP_RETURN_ON_ERROR(stop_wheel((car_wheel_t)i), "car_control",
                            "stop wheel failed");
    }
    return ESP_OK;
}
