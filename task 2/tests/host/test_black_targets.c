...
static int independent_detection_and_mask(void)
{
    ...
    CHECK(bt_detector_process_rgb565(&d, pixels, W, H, &r) == ESP_OK);
    CHECK(!d.white_left_mask_active && d.orange_right_mask_active && r.target_valid);
    CHECK(r.target.x > W * 70 / 100);
    clear_frame();
    black_box(60, 100, 20, 10);
    CHECK(bt_detector_process_rgb565(&d, pixels, W, H, &r) == ESP_OK);
    CHECK(!r.target_valid);
    /* Error reset must not bring the old white endpoint back on the orange leg. */
    bt_detector_init(&d);
    d.white_delivery_complete = true;
    CHECK(bt_detector_process_rgb565(&d, pixels, W, H, &r) == ESP_OK);
    CHECK(d.orange_right_mask_active && !r.target_valid);
    return 0;
}
...
int main(void)
{
    ...
    puts("PASS: target without ball; mask acquisition/retention/restoration; "
         ...
         "independent parking; noise/location rejection; orange right mask; "
