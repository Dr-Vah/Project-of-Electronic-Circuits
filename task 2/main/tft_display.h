...
/* Temporary camera tuning page: an already-binarized RGB565 frame with its
 * four coloured channel boxes, plus the four per-channel measurements. */
esp_err_t tft_display_show_camera_debug(const uint16_t *pixels,
                                        uint16_t width, uint16_t height,
                                        const uint8_t dark_percent[4],
                                        const bool black[4]);

/** Scale and show a full-colour camera frame on the complete display. */
