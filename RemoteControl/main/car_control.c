...
static SemaphoreHandle_t s_motor_lock;
...
{
    (void)arg;
    xSemaphoreTake(s_motor_lock, portMAX_DELAY);
...
esp_err_t car_control_init(const car_control_config_t *config)
{
    ...
    s_motor_lock = xSemaphoreCreateMutex();
    if (!s_motor_lock) return ESP_ERR_NO_MEM;
                                   ...
                                   float omega_rad_s)
{
    ...
    xSemaphoreTake(s_motor_lock, portMAX_DELAY);
    for (int i = 0; i < CAR_WHEEL_COUNT; ++i)
        s_target_speed_mps[i] = clampf(wheel_speed[i],
            -s_config.max_wheel_speed_mps, s_config.max_wheel_speed_mps);
    xSemaphoreGive(s_motor_lock);
    return ESP_OK;
}
