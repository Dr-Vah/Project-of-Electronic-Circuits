...
#define BALL_START_DELAY_MS           5000
                                     ...
                                     int64_t now_us)
{
    ...
    context->reacquire_last_sequence = 0U;
                                        ...
                                        tracking_context_t *context)
{
    ...
    if (state_is_searching(context->state)) {
        ...
        const bool fresh_frame = ir->frame_sequence != 0U &&
            ir->frame_sequence != context->reacquire_last_sequence;
        if (fresh_frame) {
            context->reacquire_last_sequence = ir->frame_sequence;
            if (now_us >= context->ignore_line_until_us &&
                middle_sees_line) {
    ...
    if (ir->black_count == 0) {
        ...
        if (now_us < context->recovery_guard_until_us ||
            ...
            /* Do not keep executing the previous forward/arc command while
             * waiting to decide whether this is a real corner. */
            ESP_ERROR_CHECK(car_control_stop());
                             ...
                             const infrared_data_t *ir, int64_t now_us)
{
    ...
    case AVOID_STRAFE_LEFT:
        if (now_us >= avoidance->next_sample_us) {
            ...
            const bool obstacle_is_clear =
                (avoidance->latest_sample.valid &&
                 avoidance->latest_sample.distance_cm >=
                     OBSTACLE_CLEAR_CM) ||
                avoidance->latest_sample.status ==
...
void app_main(void)
{
    ...
    /* Keep the UVC stream alive, but hold the chassis still for the requested
     * five seconds before handing motor ownership to the ball controller. */
    ESP_LOGI(TAG, "line course complete; waiting %d ms before ball mission",
             BALL_START_DELAY_MS);
    vTaskDelay(pdMS_TO_TICKS(BALL_START_DELAY_MS));
    ESP_ERROR_CHECK(ball_transport_controller_start());
    camera_line_sensor_enable_ball_mode();
    ESP_LOGI(TAG, "WhiteBallDetection enabled; starting ball search and "
                 "transport");
}
