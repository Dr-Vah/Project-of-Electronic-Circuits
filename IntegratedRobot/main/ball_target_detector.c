...
#define BT_BALL_REQUIRED_FRAMES 1
#define BT_BALL_MAX_CENTER_JUMP 24
#define BT_ORANGE_REQUIRED_FRAMES 1
...
static uint8_t bt_black_ahead_percent(const bt_preprocessed_frame_t *frame)
{
    ...
    const int x0 = bt_max(0, (int)(width *
                                      ...
                                      bt_raw_ball_result_t *result)
{
    ...
    /* Normalize only the white-ball luminance/chroma planes. Black/orange
     * thresholds must continue to see the original camera intensities. */
    uint32_t histogram[256] = {0};
    for (size_t i = 0; i < pixel_count; ++i) ++histogram[gray[i]];
    uint32_t cumulative = 0;
    int background = 255;
    for (int i = 0; i < 256; ++i) {
        cumulative += histogram[i];
        if (cumulative >= pixel_count * 3 / 4) { background = i; break; }
    }
    uint8_t *normalized = NULL;
    if (background >= 60 && background < 180) {
        normalized = bt_alloc(pixel_count * 2, false,
                              MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
        if (normalized == NULL) return ESP_ERR_NO_MEM;
        const int gain = bt_min(384, 180 * 256 / background);
        for (size_t i = 0; i < pixel_count; ++i) {
            normalized[i] = (uint8_t)bt_min(255, gray[i] * gain / 256);
            normalized[pixel_count + i] =
                (uint8_t)bt_min(255, chroma[i] * gain / 256);
        }
        gray = normalized;
        chroma = normalized + pixel_count;
    }
                                         ...
                                         bt_raw_target_result_t *result)
{
    ...
    for (int seed_y = roi_y0; seed_y < roi_y1; ++seed_y) {
        for (int seed_x = roi_x0; seed_x < roi_x1; ++seed_x) {
            ...
            int distance = 0;
            if (hint != NULL) {
                const int dx = center_x - (int)hint->x * width / frame->source_width;
                const int dy = center_y - (int)hint->y * height / frame->source_height;
                distance = dx * dx + dy * dy;
                /* Low frame rates can produce large inter-frame motion. */
                if (distance >= best_distance) continue;
            }
            const int32_t score = area * 3 +
                (BT_DARK_THRESHOLD - mean) * 3 + center_y;
            if (hint == NULL) {
                if (!best[0].valid || score > best[0].score) {
                    best[1] = best[0];
                } else {
                    side = 1;
                }
            }
            if (hint != NULL || !best[side].valid || score > best[side].score) {
    ...
    /* Acquisition uses the two strongest blocks anywhere in the ROI.
     * Select the opposite candidate from the previous build: the image-left
     * block is the desired endpoint for the current camera mounting. Once
     * selected, follow that same block across the image centre. */
    if (hint == NULL && best[1].valid && best[0].x > best[1].x) {
        const bt_target_candidate_t temporary = best[0];
        best[0] = best[1];
        best[1] = temporary;
    }
                                   ...
                                   bt_ball_t *stable_ball)
{
    ...
    if (!near) {
        ...
        if (stable_ball != NULL) *stable_ball = detected;
        return true;
    }
                                     ...
                                     bt_target_t *stable_target)
{
    ...
    if (!near) {
        ...
        if (stable_target != NULL) *stable_target = detected;
        return true;
    }
                                     ...
                                     bt_detector_result_t *result)
{
    ...
    if (detector->white_delivery_complete && !detector->orange_phase_active) {
        detector->white_left_mask_active = false;
        detector->orange_phase_active = true;
    ...
    result->black_ahead_percent = bt_black_ahead_percent(&frame);
    ...
    /* A low-rate camera can move farther than the local window in one frame.
     * Retry globally now, rather than throwing away a usable observation. */
    if (!raw_ball.found && detector->have_ball_hint) {
        error = bt_detect_white_ball(&frame, NULL, &raw_ball);
        if (error != ESP_OK) {
            bt_preprocess_release(&frame);
            return error;
        }
    ...
    error = bt_detect_black_targets(&frame,
        ...
        detector->white_left_mask_active, &raw_targets);
    if (error != ESP_OK) {
        bt_preprocess_release(&frame);
        return error;
    }
    ...
    if (!detector->white_delivery_complete &&
        ...
        detector->target_valid = false;
        detector->target_consecutive_frames = 0;
        error = bt_detect_black_targets(&frame, NULL, true, &raw_targets);
        if (error != ESP_OK) {
            bt_preprocess_release(&frame);
            return error;
        }
