...
static esp_err_t tft_draw_text_line_scaled(int x, int y, const char *text,
                                           uint16_t foreground,
                                           uint16_t background,
                                           uint8_t scale)
{
    ...
    const uint16_t pixel_width = 5U * scale;
    const uint16_t pixel_height = 7U * scale;
    const uint16_t character_pitch = 6U * scale;
    if (scale == 0 || scale > FONT_SCALE || x < 0 || y < 0 || x >= TFT_WIDTH ||
        y + pixel_height > TFT_HEIGHT) {
    ...
    for (uint16_t pixel_y = 0; pixel_y < pixel_height; ++pixel_y) {
        for (uint16_t pixel_x = 0; pixel_x < width; ++pixel_x) {
            ...
            if (character_index < text_length && character_x < pixel_width) {
                const char character = text[character_index];
                if (character >= 32 && character <= 126) {
                    const uint8_t *glyph = s_font_5x7[(int)character - 32];
                    const int source_x = character_x / scale;
                    const int source_y = pixel_y / scale;
    ...
    esp_err_t result = tft_set_window((uint16_t)x, (uint16_t)y,
                                      (uint16_t)(x + width - 1),
                                      (uint16_t)(y + pixel_height - 1));
    if (result != ESP_OK) {
        return result;
    }
    return tft_write(true, s_tft_text_buffer,
                     width * pixel_height * TFT_BYTES_PER_PIXEL);
}

static esp_err_t tft_draw_text_line(int x, int y, const char *text,
                                    uint16_t foreground, uint16_t background)
{
    return tft_draw_text_line_scaled(x, y, text, foreground, background,
                                     FONT_SCALE);
}
...
esp_err_t tft_display_update(const tft_display_data_t *data)
{
    ...
    snprintf(line, sizeof(line), "A:%+4.0fRPM",
             isfinite(data->wheel_a_rpm) ? data->wheel_a_rpm : 0.0f);
    ...
    snprintf(line, sizeof(line), "B:%+4.0fRPM",
             isfinite(data->wheel_b_rpm) ? data->wheel_b_rpm : 0.0f);
    ...
    snprintf(line, sizeof(line), "D:%+4.0fRPM",
             isfinite(data->wheel_d_rpm) ? data->wheel_d_rpm : 0.0f);
    ...
    if (!isfinite(data->distance_cm) || data->distance_cm < 0.0f) {
        strcpy(line, "NO ECHO");
    } else {
...
{
    if (!isfinite(TFT_DISPLAY_WHEEL_DIAMETER_M) ||
        TFT_DISPLAY_WHEEL_DIAMETER_M <= 0.0f) {
        return ESP_ERR_INVALID_ARG;
    }

    float wheel_speed_mps[CAR_WHEEL_COUNT];
    ESP_RETURN_ON_ERROR(car_control_get_wheel_speeds(wheel_speed_mps), TAG,
                        "read measured wheel speeds failed");

    const float metres_per_revolution =
        3.14159265358979323846f * TFT_DISPLAY_WHEEL_DIAMETER_M;
    const float mps_to_rpm = 60.0f / metres_per_revolution;
    const tft_display_data_t display_data = {
        .wheel_a_rpm = wheel_speed_mps[CAR_WHEEL_A] * mps_to_rpm,
        .wheel_b_rpm = wheel_speed_mps[CAR_WHEEL_B] * mps_to_rpm,
        .wheel_d_rpm = wheel_speed_mps[CAR_WHEEL_D] * mps_to_rpm,
        .distance_cm = ultrasonic_sample != NULL && ultrasonic_sample->valid
                           ? ultrasonic_sample->distance_cm
                           : -1.0f,
    };
    return tft_display_update(&display_data);
}

esp_err_t tft_display_show_camera_debug(const uint16_t *pixels,
                                        uint16_t width, uint16_t height,
                                        const uint8_t dark_percent[4],
                                        const bool black[4])
{
    enum { DEBUG_IMAGE_HEIGHT = 120 };
    if (pixels == NULL || dark_percent == NULL || black == NULL || width == 0 ||
        height == 0) {
        return ESP_ERR_INVALID_ARG;
    }

    /* Scale the decoded frame to leave 40 pixels for concise measurements. */
    ESP_RETURN_ON_ERROR(tft_set_window(0, 0, TFT_WIDTH - 1,
                                       DEBUG_IMAGE_HEIGHT - 1), TAG,
                        "set camera window failed");
    for (uint16_t y = 0; y < DEBUG_IMAGE_HEIGHT; ++y) {
        const uint16_t source_y = (uint32_t)y * height / DEBUG_IMAGE_HEIGHT;
        uint16_t *line = (uint16_t *)s_tft_line_buffer;
        for (uint16_t x = 0; x < TFT_WIDTH; ++x) {
            const uint16_t source_x = (uint32_t)x * width / TFT_WIDTH;
            line[x] = pixels[(size_t)source_y * width + source_x];
        }
        ESP_RETURN_ON_ERROR(tft_write(true, s_tft_line_buffer,
                                      sizeof(s_tft_line_buffer)), TAG,
                            "draw camera row failed");
    }

    const uint16_t black_color = rgb565(0, 0, 0);
    const uint16_t white = rgb565(255, 255, 255);
    const uint16_t cyan = rgb565(0, 220, 255);
    const uint16_t yellow = rgb565(255, 220, 0);
    const uint16_t colours[4] = {cyan, yellow, rgb565(255, 0, 255),
                                 rgb565(0, 255, 100)};
    char line[32];
    ...
    snprintf(line, sizeof(line), "1:%3u%%%c 2:%3u%%%c", dark_percent[0],
             black[0] ? '*' : '-', dark_percent[1], black[1] ? '*' : '-');
    ESP_RETURN_ON_ERROR(tft_draw_text_line_scaled(2, 136, line, colours[0],
                                                   black_color, 1), TAG,
                        "draw camera channels 1/2 failed");
    snprintf(line, sizeof(line), "3:%3u%%%c 4:%3u%%%c", dark_percent[2],
             black[2] ? '*' : '-', dark_percent[3], black[3] ? '*' : '-');
    return tft_draw_text_line_scaled(2, 148, line, colours[2], black_color, 1);
}

esp_err_t tft_display_show_camera_frame(const uint16_t *pixels,
                                        uint16_t width, uint16_t height)
{
    ...
    ESP_RETURN_ON_ERROR(tft_set_window(0, 0, TFT_WIDTH - 1, TFT_HEIGHT - 1),
                        TAG, "set full camera window failed");
    for (uint16_t y = 0; y < TFT_HEIGHT; ++y) {
        const uint16_t source_y = (uint32_t)y * height / TFT_HEIGHT;
        uint16_t *line = (uint16_t *)s_tft_line_buffer;
        for (uint16_t x = 0; x < TFT_WIDTH; ++x) {
            const uint16_t source_x = (uint32_t)x * width / TFT_WIDTH;
            line[x] = pixels[(size_t)source_y * width + source_x];
