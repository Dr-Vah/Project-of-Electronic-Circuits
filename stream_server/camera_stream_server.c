...
static const char s_index_html[] =
    ...
    "img{width:100%;height:auto;image-rendering:auto;background:#000;border-radius:8px}"
    "#s{color:#9bd;margin:10px}</style></head><body><main>"
    "<h1>ESP32 摄像头实时画面</h1><div id=s>等待视频帧…</div>"
    "<img src=/stream alt='camera stream'>"
    "<script>setInterval(async()=>{try{let r=await fetch('/status',{cache:'no-store'});"
    "let j=await r.json();document.querySelector('#s').textContent="
    "`已接收 ${j.frames} 帧 · 当前 JPEG ${j.bytes} 字节`}catch(e){}},1000)</script>"
    "</main></body></html>";
...
static esp_err_t status_handler(httpd_req_t *request)
{
    ...
    const int written = snprintf(json, sizeof(json),
                                 "{\"frames\":%" PRIu32 ",\"bytes\":%u}",
                                 sequence, (unsigned)length);
...
static esp_err_t stream_handler(httpd_req_t *request)
{
    ...
    while (result == ESP_OK) {
        ...
        if (required == 0 || sequence == last_sequence) {
