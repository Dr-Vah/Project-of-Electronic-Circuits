...
Page({
  data:{status:'未连接',connected:false,connecting:false,driving:false,percent:60,
    sensor:'等待传感器',sensorDetail:'',useCompass:true,tiltLatched:false,details:false,x:'0.00',y:'0.00',w:'0.00',mode:'joystick',joyX:0,joyY:0,
    ...
    dizzy:0,fatigue:0,idle:0,manual:-1,fault:false,faceOptions:['自动情绪',...C.FACES],faceIndex:0,navPadTop:56},
  onLoad() {
    try{const r=wx.getMenuButtonBoundingClientRect();if(r&&r.bottom)this.setData({navPadTop:r.bottom+8});}catch(e){}
    ...
    this.connection=r=>{if(r.deviceId===this.deviceId&&!r.connected&&this.data.connected)this.disconnect('蓝牙断开，请重新连接');};
  ...
  async connect() {
    ...
    try {
      // 重连前先重置蓝牙适配器，避免上次连接未完全关闭导致 createBLEConnection 失败
